 ...
 */
export function buildRigidSemanticWeights(geometry, skeleton, resolveJoint) {
    const position = geometry.getAttribute("position");
    if (!position)
        throw new Error("cannot bind geometry without position attribute");
    if (skeleton.skeleton.bones.length > 255)
        throw new Error("compact semantic weights support at most 255 bones");
    ...
    for (let vertex = 0; vertex < position.count; vertex += 1) {
        ...
        if (boneIndex === undefined)
            throw new Error(`semantic weight resolver returned unknown joint: ${joint}`);
...
export function buildSemanticWeights(geometry, skeleton, joints, maxInfluences = 4) {
    const position = geometry.getAttribute("position");
    if (!position)
        throw new Error("cannot bind geometry without position attribute");
    ...
    return { indices: geometry.getAttribute("skinIndex"), weights: geometry.getAttribute("skinWeight"), maxWeightError };
}
export function validateWeights(geometry, tolerance = 0.001) {
    const weights = geometry.getAttribute("skinWeight");
    const indices = geometry.getAttribute("skinIndex");
    const errors = [];
    if (!weights || weights.itemSize !== 4)
        errors.push("skinWeight attribute is missing or not vec4");
    if (!indices || indices.itemSize !== 4)
        errors.push("skinIndex attribute is missing or not vec4");
    if (weights)
        for (let index = 0; index < weights.count; index += 1) {
            ...
            if (Math.abs(sum - 1) > tolerance)
                errors.push(`skin weights do not sum to one at vertex ${index}: ${sum}`);
        }
    return errors;
}

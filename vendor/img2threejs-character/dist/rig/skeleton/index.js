...
export function buildSkeleton(graph) {
    const bones = new Map();
    ...
    while (pending.length) {
        ...
        if (parentId && !bones.has(parentId)) {
            pending.push(joint);
            if (pending.every((candidate) => candidate.id !== parentId))
                throw new Error(`rig joint ${joint.id} references missing parent ${parentId}`);
            continue;
        }
    ...
    if (roots.length !== 1)
        throw new Error(`Character rig must have exactly one root, got ${roots.length}`);
...
export function resetSkeleton(result) {
...
export function jointWorldPosition(result, id) {
    const bone = result.bones.get(id);
    if (!bone)
        throw new Error(`unknown rig joint: ${id}`);
    result.root.updateMatrixWorld(true);
    return bone.getWorldPosition(new THREE.Vector3());
}
export function validateRigGraph(graph) {
    const errors = [];
    const ids = new Set();
    for (const joint of graph.joints) {
        if (ids.has(joint.id))
            errors.push(`duplicate joint: ${joint.id}`);
        ids.add(joint.id);
        if (joint.parentId === joint.id)
            errors.push(`joint is its own parent: ${joint.id}`);
        if (joint.restPosition.some((value) => !Number.isFinite(value)))
            errors.push(`joint ${joint.id} has invalid rest position`);
        if (joint.parentId && !graph.joints.some((candidate) => candidate.id === joint.parentId))
            errors.push(`missing parent ${joint.parentId} for ${joint.id}`);
    }
    if (graph.joints.filter((joint) => !joint.parentId).length !== 1)
        errors.push("rig must have one root joint");
    return errors;
}
export function asVector(position) {

...
export function createCharacterForArchetype(id, options = {}) {
    const descriptor = createDefaultArchetypes().get(id);
    if (!descriptor && !options.archetype)
    ...
    if (options.traits?.length)
        ir.archetype.traits = [...new Set([...ir.archetype.traits, ...options.traits])];
    if (id === "mechanical")
        applyMechanicalAppearance(ir);
    if (id === "reptilian")
        applyReptilianAppearance(ir);
    if (id === "multi-limb")
        addExtraArmAppendages(ir);
    if (id === "quadruped")
        addQuadrupedAppendages(ir);
    if (id === "serpentine")
        addSerpentineAppendage(ir);
    return ir;
}
function applyMechanicalAppearance(ir) {
    const skin = ir.appearanceGraph.materials.find((material) => material.id === "skin");
    if (skin) {
        skin.semanticType = "armor";
        skin.baseColor = [0.22, 0.27, 0.34];
        skin.roughness = 0.32;
        skin.metalness = 0.72;
        skin.clearcoat = 0.42;
    }
...
function applyReptilianAppearance(ir) {
...
function addExtraArmAppendages(ir) {
    const extras = [
...
function addQuadrupedAppendages(ir) {
    const legs = [
...
function addSerpentineAppendage(ir) {
    const points = [[0, 0.58, 0], [0.12, 0.45, -0.05], [-0.08, 0.28, -0.16], [0.04, 0.12, -0.3]];
    const item = { id: "serpentine-body", rootRegion: "pelvis", semanticRole: "serpentine-body", geometryMode: "tube", materialId: "skin", path: { id: "serpentine-body-axis", role: "serpentine-axis", points } };

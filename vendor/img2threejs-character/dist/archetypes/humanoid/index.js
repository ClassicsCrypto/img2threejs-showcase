...
const Q = [0, 0, 0, 1];
export function createHumanoidCharacterIR(options = {}) {
    ...
    for (const [id, kind] of anatomy)
        addSemanticNode(ir.semanticGraph, { id, kind });
    const parentRelations = [["character", "skull"], ["character", "neck"], ["character", "thorax"], ["character", "abdomen"], ["character", "pelvis"], ["character", "left-arm"], ["character", "right-arm"], ["character", "left-leg"], ["character", "right-leg"], ["left-arm", "left-hand"], ["right-arm", "right-hand"], ["left-leg", "left-foot"], ["right-leg", "right-foot"], ["skull", "face"], ["skull", "eyes"], ["skull", "scalp"]];
    for (const [from, to] of parentRelations)
        addSemanticRelation(ir.semanticGraph, from, to, "parent");
    ...
    ir.shapeGraph.lofts.push(loft("torso", "thorax", [
    ...
    ], "skin"), loft("neck", "neck", [section(0, [0, 0.94, 0], 0.052, 0.047), section(1, [0, 1.01, 0], 0.045, 0.043)], "skin"), loft("left-upper-arm", "left-arm", armSections(1), "skin"), loft("right-upper-arm", "right-arm", armSections(-1), "skin"), loft("left-leg", "left-leg", legSections(1), "skin"), loft("right-leg", "right-leg", legSections(-1), "skin"), loft("left-hand", "left-hand", handSections(1), "skin"), loft("right-hand", "right-hand", handSections(-1), "skin"), loft("left-foot", "left-foot", footSections(1), "skin"), loft("right-foot", "right-foot", footSections(-1), "skin"), loft("head", "skull", [section(0, [0, 1.01, 0], 0.08, 0.07), section(0.5, [0, 1.08, 0], 0.088, 0.077), section(1, [0, 1.16, 0.005], 0.075, 0.068)], "skin"));
    ...
    if (options.addShirt !== false)
        ir.wearableGraph = { items: [{ id: "shirt", kind: "shirt", covers: ["thorax", "abdomen"], attachmentMode: "skins-with", offset: 0.008, materialId: "cloth", seamIds: ["shirt-collar", "shirt-sleeve"], foldStrength: 0.35 }, { id: "shorts", kind: "shorts", covers: ["pelvis"], attachmentMode: "skins-with", offset: 0.01, materialId: "cloth", seamIds: ["waistband"], foldStrength: 0.25 }] };
    ...
    if (options.profile === "hero")
        ir.meta.assumptions.push("Hero appearance backends remain capability-gated; SSS/TSL are optional at runtime.");
    return ir;
}
...
function loft(id, region, sections, materialId) {
    return { id, region, axis: { id: `${id}-axis`, role: "anatomical-axis", points: sections.map((item) => item.center) }, sections, continuityConstraints: [], topologyIntent: "deformable-organic", materialId };
}
...
function handSections(side) {
    const orientation = side === 1 ? [0, 0, -Math.SQRT1_2, Math.SQRT1_2] : [0, 0, Math.SQRT1_2, Math.SQRT1_2];
...
function footSections(side) {
    const orientation = [Math.SQRT1_2, 0, 0, Math.SQRT1_2];
...
function humanoidRig() {
    const joint = (id, parentId, role, restPosition) => [id, parentId, role, restPosition];
    const values = [
    ...
    return {
        joints: values.map(([id, parentId, role, restPosition]) => ({ id, parentId, role, restPosition, restRotation: Q, axis: [0, 1, 0] })),
...
function defaultHair() {
    const points = [[-0.055, 1.14, 0.005], [-0.035, 1.17, 0.03], [0, 1.18, 0.045], [0.035, 1.17, 0.03], [0.055, 1.14, 0.005], [0, 1.16, -0.03]];
    ...
    const guides = points.map((point, index) => ({ id: `hair-guide-${index}`, rootId: `hair-root-${index}`, points: [point, [point[0] * 1.15, point[1] + 0.035, point[2] + 0.01], [point[0] * 1.3, point[1] - 0.005, point[2] + (point[2] >= 0 ? 0.06 : -0.06)]], taper: 0.4 }));
    return { id: "hair", kind: "hair", materialId: "hair", representation: "tube", lod: [1, 0.4], flow: { emitter: "scalp", roots, directionalField: { id: "crown-flow", type: "linear", parameters: { lateralAttraction: -0.2, upward: 0.2, frontFlow: 0.25, rearFlow: -0.25 } }, guides, clumps: [{ id: "hair-clump-0", guideIds: guides.map((guide) => guide.id), spread: 0.02, strandCount: 8 }], modifiers: { gravity: 0.15, attraction: 0.25, spread: 0.3 } } };
}
